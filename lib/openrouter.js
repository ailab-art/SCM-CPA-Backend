...
// True only for the specific "this model isn't free anymore" 404 OpenRouter
// sends back — worth distinguishing from a real error (bad request, auth
// failure, model genuinely doesn't exist) so those still surface immediately
// instead of silently trying three more models first.
function isModelUnavailableForFree(status, bodyText) {
  if (status !== 404) return false;
  return /unavailable for free|not available for free/i.test(bodyText);
}
...
export async function generateResume(fields) {
  ...
  for (const model of CANDIDATE_MODELS) {
    try {
      ...
      lastError = err;
      if (isModelUnavailableForFree(err.status, err.bodyText)) {
        // Try the next model in the list.
        continue;
      }
      // Any other failure (bad request, auth, rate limit, empty response) —
      // don't waste three more round-trips on models that won't fix it.
      throw err;
    }
  }
